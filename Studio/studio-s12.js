...
function evaluate(program) { 
    ...
    while (! is_null(C)) {
        ...
        if (is_literal(command)) {
            ...
            if (is_null(scan_out_declarations(conditional_consequent(command))) || is_null(scan_out_declarations(conditional_consequent(command)))){
                return error("name not declared");
            }else{
